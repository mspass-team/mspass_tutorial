...
# this checkpoint corresponds to the PhaseNet-Scale model.
ckpt_fname = f'new_phasenet_best_{filter_factor}_factor.pt'
...
def parse_args():
    p = argparse.ArgumentParser(description="PhaseNet-Scale training entrypoint")
    p.add_argument("--year",
                   type=str,
                   default="all",
...
def main():
    ...
    model.default_args.update({"blinding": (200, 200)})
